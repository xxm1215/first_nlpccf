...
class FakeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.df.iloc[idx]['2_way_label']
...
def train():
    ...
    for epoch in range(num_epoch):
        ...
        for idx, batch in enumerate(train_loader):
            ...
            image_features = image_features.reshape(image_features.size(0), -1)  # Reshape image_features
            text_features = text_features.reshape(text_features.size(0), -1)  # Reshape text_features

            # Pass image_features and text_features through linear layer
            linear_layer1 = nn.Linear(2048 * 7 * 7, 512)
            linear_layer2 = nn.Linear(768, 512)
            image_features = linear_layer1(image_features)  # Pass image_features through linear layer
            text_features = linear_layer2(text_features)  # Pass text_features through linear layer
...
def test(multiheadAttentionModel, test_loader):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            text_features = torch.squeeze(text_features)  # (64,1,512)-->(64,512)
