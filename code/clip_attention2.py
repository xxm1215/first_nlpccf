...
def train():
    ...
    for epoch in range(num_epoch):
        ...
        print("macro F1 is: ", f1_macro)
        # fold_test_f1.append(f1_macro)
        if f1_macro > float(test_F1):
            test_F1 = f1_macro
        ...
        saved_model.save_pretrained("/media/qust521/qust_3_big/fake_news_data/weibo/nlpccf/weibo/trained_model.model")
        ...
        if epoch-saved_epoch >= tolerant:
            ...
            del model, saved_model, optimizer,pre_detection, loss_detection, labels, train_loader, test_loader, loss_detection_total
            gc.collect()
            torch.cuda.empty_cache()
            test_F1 = max(test_F1)
            with open("./test_finetune_t1.txt", 'w') as outf:
                outf.write("Best F1 score: {} at epoch {}\n".format(test_F1, epoch + 1))
...
def test(multiheadAttentionModel, test_loader):
    ...
    with torch.no_grad():
        ...
        #     saved_model = model
        #     saved_epoch = epoch
        #     print("saving model with val_f1 {} at Epoch {}".format(test_F1, saved_epoch + 1))
